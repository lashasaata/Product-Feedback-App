...
export default function Feedback({feedback, onClick, isUpvoted}){
    ...
    <p className="text-xs font-bold tracking-tighter text-blue-900 lg:text-lg">{feedback.title}</p>
    <p className="text-xs text-blue-gray-600 lg:text-base">{feedback.description}</p>
    ...
    <div className="flex justify-between">
        <button className={`flex hover:bg-blue-200 gap-2 items-center rounded-lg ${isUpvoted ? 'bg-blue-600 text-white' : 'bg-slate-100'} py-1 px-4`}    onClick={onClick}>
            <img src={ArrowUp} alt="arrow up icon" />
            <p className={`text-x font-bold tracking-tighter ${isUpvoted ? 'text-white' : 'text-blue-900'}`}>{feedback.upvotes}</p>
        </button>
