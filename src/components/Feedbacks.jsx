...
export default function Feedbacks() {
  ...
  const seggestedFeedbacks = data.productRequests.filter(
    (feedback) => feedback.status === "suggestion"
  );
  const uniqueCategories = [
    ...new Set(seggestedFeedbacks.map((feedback) => feedback.category)),
  ];
  ...
  const handleUpvote = (id) => {
    if (!upvotedFeedbacks.includes(id)) {
      ...
      setData({ ...data, productRequests: updatedData });
      const filteredUpvotedFeedbacks = upvotedFeedbacks.filter(
  ...
  return (
    <div className="md:bg-slate-100 md:px-[39px] md:py-[56px] md:flex md:flex-col md:gap-6 lg:flex-row lg:px-[165px] lg:py-[96px]">
      ...
      {showSortOptions && (
        <div
          className="absolute rounded-lg bg-white cursor-pointer left-[58px] top-[112px] md:left-[44%] md:top-[371px] lg:left-[56%] lg:top-[145px]"
          style={{ boxShadow: "0 10px 40px -7px rgba(55, 63, 104, 0.35)" }}
        >
          <SortOption
            onClick={() => {
              sortFeedbacks("mostUpvotes");
              setShowSortOptions(false);
            }}
            textContent={"Most Upvotes"}
          />
          <SortOption
            onClick={() => {
              sortFeedbacks("leastUpvotes");
              setShowSortOptions(false);
            }}
            textContent={"Least Upvotes"}
          />
          <SortOption
            onClick={() => {
              sortFeedbacks("mostComments");
              setShowSortOptions(false);
            }}
            textContent={"Most Comments"}
          />
          <SortOption
            onClick={() => {
              sortFeedbacks("leastComments");
              setShowSortOptions(false);
            }}
            textContent={"Least Comments"}
          />
        </div>
      )}

      <div
        className="h-screen p-6 bg-slate-100 flex flex-col gap-6 w-3/4"
        ...
        <div className="bg-white rounded-lg flex flex-col p-6 gap-6">
          <div className="flex justify-between items-center">
            ...
            <p
              ...
              className="text-xs md:text-xl font-bold tracking-wide text-blue-600 underline"
            >
              View
            </p>
          </div>
      ...
      <div className="lg:w-[100%] lg:flex lg:flex-col lg:gap-6">
        <div
          className="py-2 px-6 bg-sky-950 hidden justify-between items-center md:rounded-lg lg:flex"
